...
class AppAttendanceListWidget extends StatelessWidget {
  const AppAttendanceListWidget({
    super.key,
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<AttendanceApprovedList,
      ...
      builder: (_, state) {
        ...
        return state.maybeWhen(
          ...
          failure: (failure) => AppFailureWidget(
            ...
            retry: () =>
                context.cubit<AttendanceApprovedList>().request(params),
          ),
          success: (data) {
            $logger.devLog('data.length ${data.length}');
            if (data.isEmpty) {
              return AppFailureWidget(
                errorMsg: 'No Employee Checkins Found',
                retry: () {
                  final inp = params;
                  context.cubit<AttendanceApprovedList>().request(inp);
                },
              );
            }
